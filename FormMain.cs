...
namespace linesk
{
    ...
    public partial class FormMain : Form
    {
        ...
        string NamePlayer = "New Gamer"; // имя игрока по умолчанию
            ...
            for (int a = 0; a < 100; a++)
            {
                if (GItems[a].Active == true)
                {
                    if (indexClick != -1)
                    {
                        ...
                        if (CheckPath(indexStart, indexEnd))
                        {
        ...
        void ComputeCellsCoordinate()
        {
            ...
            for (int x = 0; x < num; x++)
            {
                for (int y = 0; y < num; y++)
                {
                    GItems[count].CellCoordinate = new Rectangle(x * lenCell + 10,
                                                            y * lenCell + deltaY / 2 + toolStrip1.Height, lenCell, lenCell);
                    count++;
                }
            }
        ...
        //Проверка, с использованием волнового алгоритма, возможно ли перемстить элемент 
        bool CheckPath(int indexStart, int indexEnd)
        {
            int startX = indexStart % 10,
                startY = (indexStart - startX) / 10;

            int endX = indexEnd % 10,
                 endY = (indexEnd - endX) / 10;

            bool[,] temp = new bool[10,10];


            //Создание таблицы для алгоритма
            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j < 10; j++)
                {
                    temp[j, i] = GItems[i * 10 + j].Visible;
                }
            }

            WaveAlgorithm path =new WaveAlgorithm();
            path.Matrix = temp;

            return path.Check(startX, startY, endX, endY);
        }
        ...
        /// <summary>
        /// Запись всех настроек в файл .set на диск в
